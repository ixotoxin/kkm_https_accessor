...
namespace UnitTests {
    ...
    TEST_CASE("text", "[trait]") {
        for (int ch = 0; ch < 128; ++ch) {
            REQUIRE(Meta::WcsText::trimPredicate(static_cast<wchar_t>(ch)) == static_cast<bool>(std::iswspace(ch)));
            REQUIRE(Meta::WcsText::noTrimPredicate(static_cast<wchar_t>(ch)) != static_cast<bool>(std::iswspace(ch)));
            REQUIRE(Meta::WcsText::toLower(static_cast<wchar_t>(ch)) == std::towlower(ch));
        }

        for (auto ch : L"АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя"sv) {
            REQUIRE(Meta::WcsText::toLower(ch) == std::towlower(ch));
        }

        for (int ch = 0; ch < 128; ++ch) {
            REQUIRE(Meta::MbsText::trimPredicate(static_cast<char>(ch)) == static_cast<bool>(std::isspace(ch)));
            REQUIRE(Meta::MbsText::noTrimPredicate(static_cast<char>(ch)) != static_cast<bool>(std::isspace(ch)));
            REQUIRE(Meta::MbsText::toLower(static_cast<char>(ch)) == std::tolower(ch));
        }

        for (auto ch : "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя"sv) {
            REQUIRE(Meta::MbsText::toLower(ch) == std::tolower(ch));
        }
    }
    ...
    TEST_CASE("text", "[trim]" ) {
        {
            ...
            for (const auto & [given, expected] : strings) {
                auto buffer = std::make_unique_for_overwrite<wchar_t[]>(given.size() + 1);
                if (given.empty()) {
                    buffer[0] = L'\0';
                } else {
                    std::wmemcpy(buffer.get(), given.data(), given.size());
                    buffer[given.size()] = L'\0';
                }
                ...
                REQUIRE(Text::trimmed/*<wchar_t>*/(chStr) == expected);
                REQUIRE(Text::trimmed/*<std::wstring>*/(stdStr) == expected);
                REQUIRE(Text::trimmed/*<std::wstring_view>*/(strView) == expected);

                auto trStr = Text::trimmedChars/*<wchar_t>*/(chStr);
                REQUIRE(trStr == expected);

                Text::trim/*<std::wstring>*/(stdStr);
                REQUIRE(stdStr == expected);
            }
        }

        {
            ...
            for (const auto & [given, expected] : strings) {
                auto buffer = std::make_unique_for_overwrite<char[]>(given.size() + 1);
                if (given.empty()) {
                    buffer[0] = '\0';
                } else {
                    std::memcpy(buffer.get(), given.data(), given.size());
                    buffer[given.size()] = '\0';
                }
                ...
                REQUIRE(Text::trimmed/*<char>*/(chStr) == expected);
                REQUIRE(Text::trimmed/*<std::string>*/(stdStr) == expected);
                REQUIRE(Text::trimmed/*<std::string_view>*/(strView) == expected);
    ...
    TEST_CASE("text", "[lower]" ) {
        {
            ...
            for (const auto & [given, expected] : strings) {
                ...
                REQUIRE(Text::lowered/*<wchar_t>*/(chStr) == expected);
                REQUIRE(Text::lowered/*<std::wstring>*/(stdStr) == expected);
                REQUIRE(Text::lowered/*<std::wstring_view>*/(strView) == expected);

                auto lwrStr = Text::loweredChars/*<wchar_t>*/(chStr);
                REQUIRE(lwrStr == expected);
...

        {
            ...
            for (const auto & [given, expected] : strings) {
                ...
                REQUIRE(Text::lowered/*<char>*/(chStr) == expected);
                REQUIRE(Text::lowered/*<std::string>*/(stdStr) == expected);
                REQUIRE(Text::lowered/*<std::string_view>*/(strView) == expected);
