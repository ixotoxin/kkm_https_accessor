    ...
    namespace Test1 {
        TEST_CASE("Dynamic resource pool operated with primitives", "[operating]") {
            ...
            for (int i { c_stCapacity + c_stOvercapacity }; i; --i) {
                ...
                REQUIRE_FALSE(static_cast<bool>(item)); // NOLINT(*-use-after-move)
                if (i > c_stOvercapacity) {
                ...
                REQUIRE_FALSE(static_cast<bool>(item3)); // NOLINT(*-use-after-move)
                if (i > c_stOvercapacity) {
...

            {
                Pool::Accessor item1 {};
                Pool::Accessor item2 {};
                REQUIRE_FALSE(static_cast<bool>(item1));
                REQUIRE_FALSE(static_cast<bool>(item2));
                item1 = std::move(item2);
                REQUIRE_FALSE(static_cast<bool>(item1));
                REQUIRE_FALSE(static_cast<bool>(item2)); // NOLINT(*-use-after-move)
                item2 = std::move(item1);
                REQUIRE_FALSE(static_cast<bool>(item1)); // NOLINT(*-use-after-move)
                REQUIRE_FALSE(static_cast<bool>(item2));
                item1 = Pool::Accessor {};
                item2 = Pool::Accessor {};
                REQUIRE_FALSE(static_cast<bool>(item1));
        ...
        TEST_CASE("Dynamic resource pool operated with structures", "[operating]") {
            ...
            for (int i { c_stCapacity + c_stOvercapacity }; i; --i) {
                ...
                if (item) {
                    ...
                    item->m_int = i;
                    (*item).m_int = i; // NOLINT
                    item->setBool(i < c_stMagicNumber);
