    ...
    namespace Test1 {
        TEST_CASE("Dynamic MPMC queue operated with primitives", "[operating]") {
            ...
            for (int i { c_stCapacity + c_stOvercapacity }; i; --i) {
                ...
                REQUIRE_FALSE(static_cast<bool>(slot)); // NOLINT(*-use-after-move)
                if (i > c_stOvercapacity) {
                ...
                REQUIRE_FALSE(static_cast<bool>(slot3)); // NOLINT(*-use-after-move)
                if (i > c_stOvercapacity) {
...

            {
                ...
                Queue::ProducerAccessor slot2 {};
                REQUIRE_FALSE(static_cast<bool>(slot1));
                REQUIRE_FALSE(static_cast<bool>(slot2));
                slot1 = std::move(slot2);
                REQUIRE_FALSE(static_cast<bool>(slot1));
                REQUIRE_FALSE(static_cast<bool>(slot2)); // NOLINT(*-use-after-move)
                slot2 = std::move(slot1);
                REQUIRE_FALSE(static_cast<bool>(slot1)); // NOLINT(*-use-after-move)
                REQUIRE_FALSE(static_cast<bool>(slot2));
        ...
        TEST_CASE("Dynamic MPMC queue operated with structures", "[operating]") {
            ...
            for (int i { c_stCapacity + c_stOvercapacity }; i; --i) {
                ...
                if (slot) {
                    ...
                    slot->m_int = i;
                    (*slot).m_int = i; // NOLINT
                    slot->setBool(i < c_stMagicNumber);
            ...
            for (int i { c_stCapacity + c_stOvercapacity }; i; --i) {
                ...
                if (slot) {
                    ...
                    REQUIRE((*slot).m_str == str); // NOLINT
                    REQUIRE(slot->m_int == i);
                    REQUIRE((*slot).m_int == i); // NOLINT
                    if (i < c_stMagicNumber) {
                        REQUIRE(slot->getBool());
                        REQUIRE((*slot).getBool()); // NOLINT
                    } else {
                        REQUIRE_FALSE(slot->getBool());
                        REQUIRE_FALSE((*slot).getBool()); // NOLINT
                    }
                }
                /** Default constr (empty slot) **/
                Queue::ConsumerAccessor slot2 {};
