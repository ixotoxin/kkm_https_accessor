...
Приложение `regmsg.exe` регистрирует ресурсы необходимые для корректного отображения сообщений в `Event Log`. Для
выполнения требуются права Администратора. Ресурсы содержатся в самом файле, поэтому не стоит его удалять, а в случае
перемещения необходимо снова его запустить для повторной регистрации.
